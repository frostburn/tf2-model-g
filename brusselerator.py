...
if __name__ == '__main__':
    ...
    r2 = x*x+y*y
    source_X = lambda t: np.exp(-(x-2)**2-y*y) / (t + 1)
    zero = lambda t: 0
    source_functions = {
        'X': source_X,
        'Y': zero,
    }
    brusselerator = Brusselerator(1.5 + np.exp(-0.2*r2)*bl_noise(x.shape), 1.5 + np.exp(-0.3*r2), dx, source_functions=source_functions)
